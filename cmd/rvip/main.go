...
import (
	...
	"github.com/easykubeio/rvip/pkg"
	"github.com/easykubeio/rvip/version"
...
func main() {
	...
	virtualIP := flag.String("vip", "192.168.0.25", "Virtual/Floating IP")
	flag.Parse()

	log.WithFields(log.Fields{"version": version.Version}).Info("rvip")
	log.WithFields(log.Fields{"id": *id, "bind": *bind, "peers": *peersList}).Info("Cluster")
	log.WithFields(log.Fields{"rvip": *virtualIP, "interface": *_interface}).Info("Network")
